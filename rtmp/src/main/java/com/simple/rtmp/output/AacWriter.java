...
public class AacWriter extends RtmpStreamWriter {
    ...
    public void write(ContentData packet) throws IOException {
        if (packet.getHeader().getMessageType() == RtmpHeader.MessageType.AUDIO) {
            ...
            if (data.length > 0) {
                ...
                KLog.w("write(): Zero-length audio data packet found; ignoring");
            }
        }
    }
    ...
    private void parseFlvSequenceHeader(byte[] data) throws IllegalArgumentException {
        ...
        KLog.i("parseFlvSequenceHeader(): AAC sampling frequency index set to: " + samplingFrequencyIndex);
        ...
        KLog.i("parseFlvSequenceHeader(): AAC channel configuration set to: " + channelConfiguration);
    ...
    private void parseAacAudioSpecificConfig(byte[] data) {
        ...
        KLog.i("parseAacAudioSpecificConfig(): AAC profile set to: " + aacProfile);
        // Next 4 bits: sampling frequence index
        samplingFrequencyIndex = ((data[2] & 0x07) << 1) | ((data[3] & 0x80) >>> 7);
        KLog.i("parseAacAudioSpecificConfig(): AAC sampling frequency index set to: " + samplingFrequencyIndex);
        // Next 4 bits: channel configuration
        channelConfiguration = ((data[3] & 0x78) >>> 3);
        KLog.i("parseAacAudioSpecificConfig(): AAC channel configuration set to: " + channelConfiguration);
