...
//        out.write(new byte[]{(byte) 0x80, 0x00, 0x07, 0x02}); // Bytes 4 - 7: Flash player version: 11.2.202.233
        ...
        KLog.d("writeC1(): Calculating digest offset");
        ...
        KLog.d("writeC1(): (real value of) digestOffset: " + digestOffset);
        
        
        KLog.d("writeC1(): recalculated digestOffset: " + absoluteDigestOffset);
        ...
        KLog.d("writeC1(): digestOffsetBytes: " + Util.toHexString(digestOffsetBytes));  //Util.unsignedInt32ToByteArray((digestOffset % 728))));

        // Create random bytes up to the digest offset point
        byte[] partBeforeDigest = new byte[absoluteDigestOffset];
        KLog.d("partBeforeDigest(): size: " + partBeforeDigest.length);
        random.nextBytes(partBeforeDigest);

        KLog.d("writeC1(): Writing timestamp and Flash Player version");
        ...
        KLog.d("partAfterDigest(): size: " + partAfterDigest.length);
        random.nextBytes(partAfterDigest);


        // Set the offset byte
//        if (digestOffset > 772) {                      
            KLog.d("copying digest offset bytes in partBeforeDigest");
            System.arraycopy(digestOffsetBytes, 0, partBeforeDigest, 772, 4);          
//        } else {
        // Implied offset of partAfterDigest is digestOffset + 32
///        KLog.d("copying digest offset bytes in partAfterDigest");
///        KLog.d(" writing to location: " + (DIGEST_OFFSET_INDICATOR_POS - digestOffset - SHA256_DIGEST_SIZE - 8));
//        System.arraycopy(digestOffsetBytes, 0, partAfterDigest, (DIGEST_OFFSET_INDICATOR_POS - digestOffset - SHA256_DIGEST_SIZE - 8), 4);
//        }

        KLog.d("writeC1(): Calculating digest");
       ...
       // KLog.d("temp: "+Util.toHexString(tempBuffer));
        ...
        tempBuffer = null;
        if (KLog.isDebug) {
            KLog.d("writeC1(): Digest: " + Util.toHexString(digest));
        }

        // Now write the packet
        KLog.d("writeC1(): writing C1 packet");
    ...
    public final void readS1(InputStream in) throws IOException {
        ...
        if (totalBytesRead != HANDSHAKE_SIZE) {
            ...
            KLog.d("readS1(): S1 total bytes read OK");
        }
    }
    ...
    public final void readS2(InputStream in) throws IOException {
        ...
        if (KLog.isDebug) {
            KLog.d("readS2(): Server time: " + Util.toHexString(sr_serverTime));
        }
        ...
        if (KLog.isDebug) {
            KLog.d("readS2(): Server version: " + Util.toHexString(s2_serverVersion));
        }
        ...
        if (totalBytesRead != remainingBytes) {
            ...
            KLog.d("readS2(): S2 total bytes read OK");
        }
